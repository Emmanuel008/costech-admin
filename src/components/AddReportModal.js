...
import '../css/AddReportModal.css';

export function AddReportModal({ onClose, onSave, editReport = null }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.date) {
      setError('Please enter report date');
      return;
    }
  ...
  return (
    <div className="add-report-overlay" onClick={onClose}>
      <div className="add-report-modal" onClick={(e) => e.stopPropagation()}>
        <div className="add-report-header">
          <h2 className="add-report-title">{editReport ? 'Edit Report' : 'Add New Report'}</h2>
          <button className="add-report-close" onClick={onClose}>
        ...
        <form className="add-report-form" onSubmit={handleSubmit}>
          {error && <div className="add-report-error">{error}</div>}
          ...
          <div className="form-group">
            <label htmlFor="add-report-date" className="form-label">
